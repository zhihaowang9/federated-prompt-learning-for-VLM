...
def load_clip_to_cpu(cfg):
    ...
    design_details = {"trainer": 'LPT',
                      "vision_depth": 0,
                      "language_depth": cfg.TRAINER.LPT.PROMPT_DEPTH_TEXT, "vision_ctx": 0,
                      "language_ctx": cfg.TRAINER.LPT.N_CTX_TEXT}
...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        n_ctx = cfg.TRAINER.LPT.N_CTX_TEXT
        ctx_init = cfg.TRAINER.LPT.CTX_INIT
...
class LPT(TrainerX):
    def check_cfg(self, cfg):
        assert cfg.TRAINER.LPT.PREC in ["fp16", "fp32", "amp"]
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.LPT.PREC == "fp32" or cfg.TRAINER.LPT.PREC == "amp":
            clip_model.float()
        ...
        self.scaler = GradScaler() if cfg.TRAINER.LPT.PREC == "amp" else None


    def forward_backward(self, idx,batch_idx, batch, **kwargs):
        image, label = self.parse_batch_train(batch)
        prec = self.cfg.TRAINER.LPT.PREC
        if prec == "amp":
            with autocast():
